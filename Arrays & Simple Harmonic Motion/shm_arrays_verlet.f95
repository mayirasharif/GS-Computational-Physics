...
x(1) = 1.0
a(1) = -(w**2.0)*x(1)
x(2) = x(1) + Tau*v(1) + (1.0/2.0)*(Tau**2)*a(1)
t(1) = 0.0
ke(1) = 0.5*m*v(1)**2.0
U(1) = 0.5*k*x(1)**2.0
E(1) = ke(1) + U(1)
...
do i=2, n-1
	t(i) = t(i-1)+Tau 
	a(i) = -w**2.0*x(i)
	x(i + 1) = 2.0*x(i) - x(i-1) + (Tau**2.0)*a(i)
	v(i) = (x(i+1) - x(i-1))/(2.0*Tau)
	ke(i) = 0.5*m*v(i)**2.0
	U(i) = 0.5*k*x(i)**2.0
	E(i) = ke(i) + U(i)
	end do

open(100, file="kinetic-energyv.dat")

open(101, file="potential-energyv.dat")

open(102, file="total-energyv.dat")

open(103, file="x-arrayv.dat")

open(104, file="velocity-arrayv.dat")

open(105, file="w.dat")

open(106, file="av.dat")
